...
void	*file_readmap(int fd, char *ptr)
{
	...
	if (!line)
		console_error_exit(ft_strjoin("Failed to read map: ", ptr));
	while (line)
	{
		map_add(node, line, y++);
		free(line);
...
static	mlx_image_t	*file_readpng_resize(mlx_t *mlx, char *file, double s)
{
	...
	t = mlx_load_png(file);
	if (!t)
		console_error_exit(ft_strjoin("Failed to load texture: ", file));
...
t_image	*file_readimage(t_game *g, char **ptr, double scale)
{
	...
	while (*ptr)
	{
		...
		if (!node->mlx_image)
			console_error_exit(ft_strjoin("Failed to load texture: ", *ptr));
